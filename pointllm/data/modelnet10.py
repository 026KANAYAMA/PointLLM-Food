...
class ModelNet10(Dataset):
    def __init__(self, split, subset_nums=-1, use_color=False):
        ...
        config_path = os.path.join(os.path.dirname(__file__), "modelnet10_config", "ModelNet10.yaml")
        ...
        self.catfile = os.path.join(os.path.dirname(__file__), "modelnet10_config", 'modelnet10_shape_names.txt')
        self.categories = [line.rstrip() for line in open(self.catfile)]

        # Load point clouds and labels from .npy files
        points_path = os.path.join(self.root, f'modelnet10_{split}_points.npy')
        labels_path = os.path.join(self.root, f'modelnet10_{split}_labels.npy')
