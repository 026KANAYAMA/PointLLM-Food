...
class UECFood3D_v1(Dataset):
    def __init__(self, split='train', subset_nums=-1, use_color=False):
        super(UECFood3D_v1, self).__init__()
        config_path = os.path.join(os.path.dirname(__file__), "uecfood3d_v1", "uecfood3d_v1.yaml")
        ...
        self.split = split  # train/testの区別は今後のためだけ残しておく

        self.catfile = os.path.join(os.path.dirname(__file__), "uecfood3d_v1", 'uecfood3d_names.txt')
        self.categories = [line.rstrip() for line in open(self.catfile)]

        # train/testの区別なく一つだけ読み込む
